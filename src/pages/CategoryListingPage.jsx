        ...
        <ul className="space-y-1.5">
          <li>
            <button
              ...
              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all border ${categoryId === 'all' ? 'bg-blue-50/50 border-brand-navy text-brand-navy font-bold shadow-sm' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-900 hover:border-gray-200'}`}
            >
              All Products
            </button>
          </li>
          {categories.map(cat => (
            <li key={cat.id}>
              <button
                ...
                className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all border ${categoryId === cat.id.toString() ? 'bg-blue-50/50 border-brand-navy text-brand-navy font-bold shadow-sm' : 'border-transparent text-gray-600 hover:bg-gray-50 hover:text-gray-900 hover:border-gray-200'}`}
              >
                {cat.name}
              </button>
            </li>
      ...
      {currentModels.length > 0 && (
        <div className="border-t border-gray-100 pt-6">
          <div className="flex items-center justify-between mb-4">
            ...
            {modelQuery && (
              <button onClick={() => handleModelChange('')} className="text-[11px] text-brand-navy hover:text-brand-navy/80 font-bold bg-blue-50/50 px-2 py-1 rounded-md transition-colors">Clear</button>
            )}
          </div>
          <div className="space-y-3 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
            {currentModels.map(model => (
              <label key={model} className="flex items-center gap-3 cursor-pointer group">
                <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${modelQuery === model ? 'border-brand-navy bg-brand-navy shadow-sm' : 'border-gray-300 group-hover:border-brand-navy bg-white'}`}>
                  {modelQuery === model && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
                </div>
                <span className={`text-sm ${modelQuery === model ? 'text-brand-navy font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{model}</span>
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
              <div className={`w-5 h-5 rounded border flex items-center justify-center transition-all ${priceQuery === opt.id ? 'border-brand-navy bg-brand-navy shadow-sm' : 'border-gray-300 group-hover:border-brand-navy bg-white'}`}>
                {priceQuery === opt.id && <Check className="w-3.5 h-3.5 text-white" strokeWidth={3} />}
              </div>
              <span className={`text-sm ${priceQuery === opt.id ? 'text-brand-navy font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{opt.label}</span>
      ...
      <div className="border-t border-gray-100 pt-6">
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={opt.id} className="flex items-center gap-3 cursor-pointer group">
              <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${sortBy === opt.id ? 'border-brand-navy bg-brand-navy shadow-sm' : 'border-gray-300 group-hover:border-brand-navy bg-white'}`}>
                {sortBy === opt.id && <div className="w-2 h-2 rounded-full bg-white" />}
              </div>
              <span className={`text-sm ${sortBy === opt.id ? 'text-brand-navy font-bold' : 'text-gray-600 group-hover:text-gray-900'}`}>{opt.label}</span>
        ...
        <div className="bg-white mx-4 lg:mx-8 rounded-3xl mt-6 relative overflow-hidden shadow-sm border border-gray-100">
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between p-6 md:px-12 md:py-10 gap-6 relative z-10">
            <div className="text-center md:text-left text-gray-900 max-w-2xl">
              <h1 className="text-3xl md:text-5xl font-extrabold mb-4 tracking-tight font-serif text-[#012980]">{categoryName}</h1>
            ...
            <div className="w-28 h-28 md:w-40 md:h-40 shrink-0 rounded-full bg-white p-2 border border-brand-navy/30 shadow-sm hidden md:block group-hover:shadow-md transition-all">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10">
              ...
              <div className="flex flex-wrap gap-2.5">
                {searchSuggestions.types.map(type => (
                  <button key={type} onClick={() => {
                    ...
                    className="px-4 py-1.5 bg-gray-50 border border-gray-200 rounded-full text-[13px] font-medium text-gray-700 hover:bg-blue-50 hover:border-brand-navy hover:text-brand-navy transition-colors">
                    {type}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {[
                  ...
                  <button key={price.id} onClick={() => handlePriceChange(price.id)}
                    className={`px-4 py-1.5 border rounded-full text-[13px] font-medium transition-colors ${priceQuery === price.id ? 'bg-brand-navy border-brand-navy text-white' : 'bg-gray-50 border-gray-200 text-gray-700 hover:bg-blue-50 hover:border-brand-navy hover:text-brand-navy'}`}>
                    {price.label}
                  </button>
                ))}
              </div>
        ...
        <div className="bg-white border-gray-100 rounded-3xl -mt-2 lg:-mt-1 mb-8 px-4 py-6 overflow-x-auto hide-scrollbar shadow-sm">
          <div className="flex gap-6 md:gap-10 justify-start md:justify-center min-w-max mx-auto px-2">
            <Link to="/category/all" className="flex flex-col items-center gap-3 group">
              <div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center border overflow-hidden transition-all ${categoryId === 'all' ? 'border-brand-navy border-2 shadow-sm bg-blue-50/50' : 'border-gray-200 bg-white group-hover:border-brand-navy group-hover:shadow-sm'}`}>
                <div className={`w-full h-full flex items-center justify-center font-extrabold text-sm text-center leading-tight ${categoryId === 'all' ? 'text-brand-navy' : 'text-gray-600 group-hover:text-brand-navy'}`}>All<br />Products</div>
              </div>
              <span className={`text-[13px] md:text-sm font-bold text-center transition-colors ${categoryId === 'all' ? 'text-brand-navy' : 'text-gray-600 group-hover:text-gray-900'}`}>All Products</span>
            </Link>
            {categories.map(cat => (
              <Link key={cat.id} to={`/category/${cat.id}`} className="flex flex-col items-center gap-3 group">
                <div className={`w-16 h-16 md:w-20 md:h-20 rounded-2xl flex items-center justify-center border overflow-hidden transition-all ${categoryId === cat.id.toString() ? 'border-brand-navy border-2 shadow-sm bg-blue-50/50' : 'border-gray-200 bg-white p-1 group-hover:border-brand-navy group-hover:shadow-sm'}`}>
                  {cat.image_url ? (
                ...
                <span className={`text-[13px] md:text-sm font-bold text-center transition-colors ${categoryId === cat.id.toString() ? 'text-brand-navy' : 'text-gray-600 group-hover:text-gray-900'}`}>{cat.name}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 bg-white p-4 md:p-5 rounded-2xl shadow-sm border border-gray-100 gap-4">
          <div className="flex items-center justify-between w-full sm:w-auto gap-4">
            <span className="text-sm font-extrabold text-[#012980] bg-blue-50/50 border border-brand-navy/30 px-4 py-2 rounded-xl">{filteredProducts.length} Items</span>

            {/* Mobile Filter Trigger */}
            <button
              onClick={() => setShowMobileFilters(true)}
              className="lg:hidden flex items-center gap-2 text-sm font-bold text-white bg-[#012980] px-5 py-2 rounded-xl shadow-md"
            >
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            <div className="flex bg-gray-50 border border-gray-200 rounded-xl p-1.5">
              <button onClick={() => setLayout('grid')} className={`px-5 py-2 text-sm font-bold rounded-lg transition-all ${layout === 'grid' ? 'bg-[#012980] text-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>Grid</button>
              <button onClick={() => setLayout('list')} className={`px-5 py-2 text-sm font-bold rounded-lg transition-all ${layout === 'list' ? 'bg-[#012980] text-white shadow-sm' : 'text-gray-600 hover:text-gray-900'}`}>List</button>
            </div>
          </div>
        </div>
            ...
            <div className={layout === 'grid' ? 'grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-6' : 'flex flex-col gap-4'}>
              ...
              {filteredProducts.length === 0 && (
                <div className="col-span-full py-24 text-center flex flex-col items-center bg-white rounded-3xl shadow-sm border border-gray-100">
                  ...
                  <h3 className="text-2xl font-extrabold text-[#012980] mb-2 font-serif">No products found</h3>
                  <p className="text-gray-500 max-w-md">Try adjusting your filters or search terms to find what you're looking for.</p>
                  <button onClick={() => { handleCategoryChange('all'); setSortBy('featured'); }} className="mt-8 bg-gradient-to-r from-brand-navy to-blue-500 text-white font-bold px-8 py-3 rounded-xl shadow-md hover:-translate-y-1 transition-all">
                    Clear Filters
                  </button>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-[60] lg:hidden flex">
          ...
          <div className="relative ml-auto w-[85%] max-w-sm bg-white h-full flex flex-col shadow-2xl transition-transform border-l border-gray-200">
            <div className="flex items-center justify-between p-6 border-b border-gray-100 bg-gray-50">
              <h2 className="text-xl font-bold text-[#012980] flex items-center gap-2 font-serif">
                <Filter className="w-5 h-5 text-brand-navy" /> Filters
              </h2>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex gap-4">
              ...
              <button
                ...
                className="flex-[2] px-4 py-3 bg-[#012980] text-white font-bold rounded-xl shadow-md hover:bg-[#033429]"
              >
                Apply Filters
              </button>
            </div>
