      ...
      <div className="bg-white rounded-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
            <input type="checkbox" name="is_default" checked={form.is_default} onChange={handleChange} className="accent-blue-500" />
            Set as default address
          </label>
        </div>
        <button onClick={() => onSave(form)}
          className="w-full mt-4 bg-brand-navy text-white font-bold py-3 rounded-xl text-sm hover:bg-blue-600 transition-colors">
          Save Address
        </button>
      </div>
    </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-4xl mx-auto">
        {/* User Profile Header */}
        <div className="bg-brand-navy text-white px-6 pt-8 pb-8 md:rounded-3xl md:mt-6 rounded-b-[2.5rem] shadow-sm">
          ...
          {tabs.map((tab) => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              className={`flex-1 py-2.5 text-xs md:text-sm font-bold rounded-lg transition-colors ${
                activeTab === tab.id ? 'bg-brand-navy text-white shadow-sm' : 'text-gray-500 hover:text-gray-700 hover:bg-gray-50'
              }`}>
              {tab.label}
            </button>
          ))}
        ...
        {activeTab === 'overview' && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                { label: 'Orders', value: orders.length, color: 'bg-blue-50 text-brand-navy' },
          ...
          <div className="space-y-3">
            {orders.length === 0 ? (
              <div className="text-center py-16">
                ...
                <Link to="/" className="mt-3 inline-block text-xs text-brand-navy font-bold">Start Shopping →</Link>
              </div>
            ) : (
              orders.map((order) => (
        ...
        {activeTab === 'addresses' && (
          <div className="space-y-3">
            <button onClick={() => setShowAddressModal(true)}
              className="w-full flex items-center justify-center gap-2 border-2 border-dashed border-blue-300 rounded-xl py-4 text-brand-navy text-sm font-semibold hover:bg-blue-50 transition-colors">
