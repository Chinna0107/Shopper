      ...
      <div className="bg-white rounded-t-3xl md:rounded-2xl w-full max-w-lg p-6 max-h-[92vh] overflow-y-auto">
        ...
        <label className="flex items-center gap-2.5 mt-4 cursor-pointer">
          <div onClick={() => setForm({ ...form, is_default: !form.is_default })}
            className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-colors ${form.is_default ? 'bg-brand-navy border-brand-navy' : 'border-gray-300'}`}>
        ...
        <button onClick={() => onSave(form)} disabled={saving}
          className="w-full mt-5 bg-brand-navy text-white font-bold py-3.5 rounded-xl text-sm hover:bg-blue-600 transition-colors disabled:opacity-60">
    ...
    <div className="min-h-screen bg-gray-50 pb-24">
      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-2xl font-serif font-bold text-brand-navy">Saved Addresses</h2>
          <button onClick={() => setShowForm(true)}
            className="flex items-center gap-2 text-sm font-bold text-white bg-brand-navy px-4 py-2.5 rounded-xl hover:bg-blue-600 transition-colors shadow-sm">
        ...
        {addresses.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 gap-4">
            ...
            <button onClick={() => setShowForm(true)}
              className="bg-brand-navy text-white text-sm font-bold px-6 py-2.5 rounded-xl hover:bg-blue-600 transition-colors">
              Add Address
            </button>
          </div>
        ) : (
