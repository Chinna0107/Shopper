      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-0 py-6 md:py-8 flex flex-col md:flex-row gap-6 md:gap-8 mt-16 md:mt-0">
        ...
        <div className="hidden md:block w-72 shrink-0">
           <div className="bg-white p-5 mb-6 rounded-3xl flex items-center gap-4 shadow-sm border border-gray-100 relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-blue-50 to-transparent rounded-bl-full pointer-events-none opacity-50 blur-xl"></div>
           ...
           <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
             <div className="border-b border-gray-100 p-5 hover:bg-gray-50 cursor-pointer font-bold text-gray-600 hover:text-brand-navy transition-all" onClick={() => navigate('/my-orders')}>
               My Orders
             </div>
             <div className="p-5 bg-blue-50 cursor-pointer font-bold text-brand-navy border-l-4 border-brand-navy transition-all flex items-center justify-between">
               <span>My Wishlist</span>
               <span className="bg-brand-navy text-white text-xs font-bold px-2 py-0.5 rounded-full shadow-sm">{wishlistProducts.length}</span>
             </div>
           </div>
        </div>
        ...
        <div className="flex-1 bg-white rounded-3xl shadow-xl border border-gray-100 min-h-[60vh] overflow-hidden">
          ...
          {wishlistProducts.length === 0 ? (
             <div className="flex flex-col items-center justify-center py-24 text-center px-4">
               <div className="mb-6 w-32 h-32 bg-blue-50 rounded-full flex items-center justify-center border border-blue-100 shadow-inner relative overflow-hidden">
                 <div className="absolute inset-0 bg-brand-navy/5 animate-pulse rounded-full"></div>
                 <Heart className="w-16 h-16 text-brand-navy drop-shadow-sm" fill="currentColor" />
               </div>
               ...
               <Link to="/category/all" className="bg-[#0b162c] text-white px-8 py-3.5 rounded-xl font-bold text-[15px] shadow-lg hover:shadow-xl hover:bg-[#15284b] hover:-translate-y-1 transition-all flex items-center gap-2">
                 <Star className="w-4 h-4 text-brand-navy" fill="currentColor" /> Explore Collections
               </Link>
             </div>
          ) : (
                  ...
                  <div key={product.id} className={`flex gap-6 p-6 ${idx !== wishlistProducts.length - 1 ? 'border-b border-gray-100' : ''} hover:bg-gray-50 transition-all relative group cursor-pointer`} onClick={() => navigate(`/product/${product.id}`)}>
                    ...
                    <div className="flex flex-col flex-grow justify-center">
                      <div className="flex justify-between items-start gap-4">
                        <div className="pr-12">
                          <h3 className="text-[16px] md:text-lg font-bold text-[#0b162c] group-hover:text-brand-navy transition-colors line-clamp-2 leading-snug mb-2 font-serif" style={{ fontFamily: 'Georgia, serif' }}>{product.name}</h3>
                          
                          <div className="flex items-center gap-2 mb-4">
                            <div className="flex items-center gap-1 bg-blue-50 border border-brand-navy/20 text-brand-navy px-2 py-0.5 rounded-full text-xs font-bold shadow-sm">
                          ...
                          <div className="flex items-baseline gap-3 mb-2">
                            ...
                            <span className="text-brand-navy font-bold text-[14px]">{discountPercent}% off</span>
                          </div>
                        </div>
                      ...
                      <div className="mt-2 flex items-center gap-2 md:opacity-0 md:translate-y-2 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300">
                         <button 
                            ...
                            className="bg-gradient-to-r from-brand-navy to-blue-500 text-white px-6 py-2.5 text-[14px] font-bold rounded-xl flex items-center gap-2 shadow-md hover:shadow-lg hover:-translate-y-0.5 transition-all"
                         >
