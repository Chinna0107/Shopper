...
function Input({ className = '', error, ...props }) {
  return <input className={`w-full px-3.5 py-2.5 border ${error ? 'border-red-500 bg-red-50 focus:ring-red-200' : 'border-gray-200 bg-white focus:border-[#036e26] focus:ring-[#036e26]/30'} rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 transition-colors ${className}`} {...props} />;
}
function Textarea({ className = '', error, ...props }) {
  return <textarea className={`w-full px-3.5 py-2.5 border ${error ? 'border-red-500 bg-red-50 focus:ring-red-200' : 'border-gray-200 bg-white focus:border-[#036e26] focus:ring-[#036e26]/30'} rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 transition-colors resize-none ${className}`} {...props} />;
}
function Select({ className = '', error, children, ...props }) {
  return <select className={`w-full px-3.5 py-2.5 border ${error ? 'border-red-500 bg-red-50 focus:ring-red-200' : 'border-gray-200 bg-white focus:border-[#036e26] focus:ring-[#036e26]/30'} rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 transition-colors ${className}`} {...props}>{children}</select>;
}
...
export function AdminProductFormPage() {
  ...
  const UploadBtn = ({ label, accept = 'image/*', onUpload, loading }) => (
    <label className="relative inline-flex items-center gap-2 cursor-pointer px-4 py-2 bg-gray-50 border border-dashed border-gray-300 rounded-xl text-sm text-gray-600 hover:bg-green-50 hover:border-[#036e26] transition-colors">
      ...
      case 'basic': return (
        <div className="space-y-5">
          ...
          <Section title="Category">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field label="Product Category" required hint={categories.length === 0 ? "You haven't selected any categories yet." : ""}>
          ...
          <Section title="Description">
            ...
            <Field label="Product Description" required hint="Full product details, features, specifications">
              <Textarea rows={5} value={form.description} onChange={e => set('description', e.target.value)} placeholder="Write a detailed product description..." error={errors.description} />
            </Field>
          </Section>
        </div>
      );
        ...
        <div className="space-y-5">
          ...
          <Section title="Gallery Images">
            <div className="flex flex-wrap gap-3 mb-3">
              ...
              <label className="w-20 h-20 rounded-xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center cursor-pointer hover:border-[#036e26] hover:bg-green-50 transition-colors relative">
            ...
            <Field label="Search Tags" hint="Press Enter to add">
              <div className="border border-gray-200 rounded-xl p-2 bg-white min-h-[48px] flex flex-wrap gap-2 focus-within:ring-2 focus-within:ring-[#036e26]/30">
                {form.search_tags.map((tag, i) => (
                  <span key={i} className="flex items-center gap-1 px-2.5 py-1 bg-green-100 text-[#036e26] rounded-lg text-xs font-semibold">
                    {tag}
          ...
          <Section title="Product Flags">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label className="flex items-center gap-3 p-3 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-colors">
                <input type="checkbox" checked={form.is_active} onChange={e => set('is_active', e.target.checked)} className="w-4 h-4 rounded text-[#036e26]" />
                <div>
              ...
              {TABS.map(tab => {
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-colors text-left ${activeTab === tab.id ? 'bg-[#036e26]/10 text-[#036e26] font-semibold' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>
          ...
          <div className="md:hidden w-full mb-4">
            <div className="flex gap-2 overflow-x-auto pb-1">
              {TABS.map(tab => {
                ...
                return (
                  <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                    className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-semibold transition-colors ${activeTab === tab.id ? 'bg-[#036e26] text-white' : 'bg-white text-gray-500 border border-gray-200'}`}>
